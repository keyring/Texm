...
MaxRectsBinPack::MaxRectsBinPack(int width, int height, bool rotate)
{
    Init(width, height, rotate);
}

void MaxRectsBinPack::Init(int width, int height, bool rotate)
{
    binWidth = width;
    binHeight = height;
    isRotate = rotate;
...
Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int &bestY, int &bestX) const
{
    ...
    for(size_t i = 0; i < freeRectangles.size(); ++i)
    {
        ...
        if (isRotate && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
        {
    ...
    int &bestShortSideFit, int &bestLongSideFit) const
{
    ...
    for(size_t i = 0; i < freeRectangles.size(); ++i)
    {
        ...
        if (freeRectangles[i].width >= width && freeRectangles[i].height >= height)
        {
            ...
            int shortSideFit = std::fmin(leftoverHoriz, leftoverVert);
            int longSideFit = std::fmax(leftoverHoriz, leftoverVert);
        ...
        if (isRotate && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
        {
            ...
            int flippedShortSideFit = std::fmin(flippedLeftoverHoriz, flippedLeftoverVert);
            int flippedLongSideFit = std::fmax(flippedLeftoverHoriz, flippedLeftoverVert);
...
int CommonIntervalLength(int i1start, int i1end, int i2start, int i2end)
{
    if (i1end < i2start || i2end < i1start)
        return 0;
    return std::fmin(i1end, i2end) - std::fmax(i1start, i2start);
}
